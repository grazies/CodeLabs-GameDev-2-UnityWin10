...
The following software packages are used within the lab. These can be optional, as you can choose to skip the tasks, but covering all tasks is recommended for a comprehensive coverage of different approaches to integrate with Windows, so we recommend you also install the following pre-requisites. 
...
3. Now we can click **Build**. You will be prompted to select a folder you are building to. Give it any project name. If you want to mimic our solution (not required) you can call it *Task1Solution*.  
...
2. Add a reference to Vungle's Unity Package, which includes a plugin
	...
	4. After selecting the package, you will see the Unity Import Dialog.  In Vungle's case, they ship one plugin for all platforms (iOS, Android, Windows). This is fine for us. Click **Import** to import all the files. Let's now review the important ones (for Windows Store and Windows Phone) configurations. 
	    - Notice the files are under a **Plugins/Metro** folder structure. 
...
Now that we have our references configured within Unity and have recompiled, we can call the code to show the ads. Let's go back to Visual Studio. 
