...
namespace Complete
{
    public class GameManager : MonoBehaviour
    {
        ...
        public Text m_HelpText;                     // Reference to the overlay Text to display upon launch to hint user to press F1 
...
#if EASYPLAY
            // Helpers during development positions tanks at aim length of each other so u don't have to move too much 
            m_CameraControl.GetComponent<Transform>().position = new Vector3(-7.65f, 0, 23.6f);
            m_CameraControl.GetComponent<Transform>().rotation = Quaternion.Euler(40, 60, 2.2f); 
            m_Tanks[0].m_SpawnPoint.position = new Vector3(-3, 0f, 30);
            m_Tanks[0].m_SpawnPoint.rotation = Quaternion.Euler(0, 210, 0);

            m_Tanks[1].m_SpawnPoint.position = new Vector3(-13.5f, 0f, 14.5f);
            m_Tanks[1].m_SpawnPoint.rotation = Quaternion.Euler(0, 24f, 0);
#endif

#if RUNINBACKGROUND
            //LAB: Windowing detection via Window Activation 
...
#elif SHOW_MS_ADS
        void OnMSAdCompleted ( object unused )
        {
            OnAdCompleted();
            m_MicrosoftAd.Request(MicrosoftAdsAppId, NextMicrosoftAd, Microsoft.UnityPlugins.AdType.Video);
        }
        ...
        void ShowSocialPrompt()
        {
            ...
            if ( canvas != null )
            {
                ...
                if (panel != null)
                {
                    ...
                    m_isDisplayingSocialPrompt = true;
                }
            }
        }
        ...
        private string GetTileMessage(bool isEndOfChallenge)
        {
            string message = string.Empty;
            if (isEndOfChallenge)
            {
                message = "A new TANKS Challenge awaits! Can you bring it?";
            }
            else
            {
                message = string.Format("It ain't over..  Player{0} is ready to come back. Score {1}-{2}",
                    (m_Tanks[0].m_Wins < m_Tanks[1].m_Wins) ? 1 : 2,
                    m_Tanks[0].m_Wins, m_Tanks[1].m_Wins
                    );
            }
            return message;
        }
